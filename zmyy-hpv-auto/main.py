...
class ZmyySeckill:
    ...
    # 获取用户信息
    def get_user(self):
        url = 'https://api.cn2030.com/sc/wx/HandlerSubscribe.ashx?act=User'
    ...
    def get_cid(self):
        ...
        for product in [27, 28]:
            url = 'https://api.cn2030.com/sc/wx/HandlerSubscribe.ashx?act=CustomerList&city=["{}","{}","{}"]&lat=&lng=&id=0&cityCode={}&product={}'.format(
    ...
    def get_subscribe_dates(self):
        ...
        if 'dates' not in self.config['zhimiao'] or not self.config['zhimiao']['dates']:
            url = 'https://api.cn2030.com/sc/wx/HandlerSubscribe.ashx?act=GetCustSubscribeDateAll&pid={}&id={}&month={}'.format(
    ...
    def get_subscribe_times(self, day):
        log('3. 获取预约时间')
        url = 'https://api.cn2030.com/sc/wx/HandlerSubscribe.ashx?act=GetCustSubscribeDateDetail&pid={}&id={}&scdate={}'.format(
    ...
    def get_order_status(self):
        url = 'https://api.cn2030.com/sc/wx/HandlerSubscribe.ashx?act=GetOrderStatus'
        res = self.get(url)
        if res.json()['status'] != 200:
            raise Exception(res.text)
        return res
