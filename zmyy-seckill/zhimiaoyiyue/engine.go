...
获取疫苗可预约日期
*/
func getAvailableDates(customerId int, productId string, month int,
...
func getDateDetail(dateDetailCh chan model.DateDetail,
	productId string, customerId int, e *ZMYYEngine,
...
func (e *ZMYYEngine) Run(customerId int, productId string, startTime time.Time) {
		...
		case detail := <-dateDetailCh:
			for i := 0; i < len(consts.ProxyIpArr); i++ {
				...
				go func(detail model.DateDetail, ip string) {
					...
					e.SecKill(detail, productId, secKillWg,
